...
if __name__ == "__main__":
    ...
    for i, data in enumerate(testset, 0):
        if i % 20 == 19:    # print every 2000 mini-batches
            print(i, sumerr, falsesample)
        adversary, flag = advLib.genAdv(data, net, 1.1)
        if not flag:
            falsesample += 1
            continue
        diff = adversary - data[0].to(device)
        sumerr = sumerr + torch.norm(diff, p=2)

    print("Finished!", sumerr, falsesample)
